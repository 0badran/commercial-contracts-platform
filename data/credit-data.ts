...
export function calculateRiskLevel(
  creditRating: CreditInfo["creditRating"]
...
export function getCreditInfo(retailerId: number): CreditInfo | null {
  return creditData.find((credit) => credit.userId === retailerId) || null;
}
...
export function updateCreditRating(retailerId: number): CreditInfo | null {
  ...
  const index = creditData.findIndex((credit) => credit.userId === retailerId);
  if (index !== -1) {
